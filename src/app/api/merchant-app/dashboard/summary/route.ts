    ...
    if (payload && payload.merchantId) {
      ...
      if (merchantDetails.length > 0 && merchantDetails[0].internalAccountId) {
        if (merchantDetails[0].status !== "active") {
          console.warn(`[API Dashboard] Auth: Merchant ${payload.merchantId} not active (status: ${merchantDetails[0].status}).`);
        ...
        console.warn(`[API Dashboard] Auth: Merchant ${payload.merchantId} not found or no internalAccountId.`);
        throw new UnauthorizedError("Merchant details not found or configuration incomplete.");
      }
    }
    console.warn("[API Dashboard] Auth: JWT valid, but merchantId missing in payload.");
    throw new UnauthorizedError("Invalid token payload.");
  } catch (error: any) {
    if (error instanceof ApiError) throw error;
    console.error("[API Dashboard] Auth: DETAILED JWT Verif. Error:", "Name:", error.name, "Msg:", error.message, "Code:", error.code);
...
const getEndOfWeekUTC = (date: Date) => {
  ...
  return new Date(Date.UTC(startOfWeek.getUTCFullYear(), startOfWeek.getUTCMonth(), startOfWeek.getUTCDate() + 6, 23, 59, 59, 999));
};
...
export async function GET(request: NextRequest) {
  try {
    const { merchantId, merchantInternalAccountId, merchantBusinessName } = await getAuthenticatedMerchantInfo(request);

    console.log(`[API Dashboard] Fetching summary for Merchant: ${merchantBusinessName} (ID: ${merchantId}, InternalLedgerAcc: ${merchantInternalAccountId})`);
    ...
    const todaySummary = await getTransactionSummaryForMerchant(merchantInternalAccountId, todayStart, todayEnd); // Pass ID
    console.log(`[API Dashboard] Today's Summary for ${merchantBusinessName}: ${JSON.stringify(todaySummary)} (Range: ${todayStart.toISOString()} - ${todayEnd.toISOString()})`);

    // 2. This Week's Summary (Mon-Sun)
    const weekStart = getStartOfWeekUTC(now);
    const weekEnd = getEndOfWeekUTC(now);
    const thisWeekSummary = await getTransactionSummaryForMerchant(merchantInternalAccountId, weekStart, weekEnd); // Pass ID
    console.log(`[API Dashboard] This Week's Summary for ${merchantBusinessName}: ${JSON.stringify(thisWeekSummary)} (Range: ${weekStart.toISOString()} - ${weekEnd.toISOString()})`);

    // 3. This Month's Summary
    const monthStart = getStartOfMonthUTC(now);
    const monthEnd = getEndOfMonthUTC(now);
    const thisMonthSummary = await getTransactionSummaryForMerchant(merchantInternalAccountId, monthStart, monthEnd); // Pass ID
    console.log(`[API Dashboard] This Month's Summary for ${merchantBusinessName}: ${JSON.stringify(thisMonthSummary)} (Range: ${monthStart.toISOString()} - ${monthEnd.toISOString()})`);
    ...
    if (error instanceof ApiError) {
      console.error(`[API Dashboard] ApiError: ${error.message}`, error.details ? JSON.stringify(error.details) : "");
      return NextResponse.json({ error: error.message, details: error.details }, { status: error.statusCode });
    }
    console.error(`[API Dashboard] Unexpected error: ${error.message || "Unknown"}`, error instanceof Error ? error.stack : JSON.stringify(error));
