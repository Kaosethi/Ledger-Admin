...
async function getAuthenticatedMerchantId(
  request: NextRequest
): Promise<string | null> {
  ...
  try {
    ...
    if (error instanceof Error) {
      console.warn(
        "[API Profile] JWT Verification Error:",
        error.name,
        error.message
      );
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.error(
      "[API Profile] JWT Secret configuration error:",
      (error as Error).message
    );
    return NextResponse.json(
      { error: "Server configuration error for token verification." },
      { status: 500 }
    );
  }
  ...
  try {
    ...
    console.log(
      "[API /merchant-app/profile] Authenticated Merchant ID:",
      merchantId
    );
    ...
    if (foundMerchants.length === 0) {
      console.warn(
        "[API /merchant-app/profile] Merchant not found in DB for ID from token:",
        merchantId
      );
    ...
    console.error(
      "[API /merchant-app/profile] Error processing profile request:",
      error
    );
    return NextResponse.json(
