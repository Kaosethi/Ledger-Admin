...
async function getAuthenticatedMerchantInfo(request: NextRequest): Promise<{
  ...
  try {
    ...
    if (payload && payload.merchantId) {
      ...
      if (merchantDetails.length > 0 && merchantDetails[0].internalAccountId) {
        if (merchantDetails[0].status !== "active") {
          console.warn(
            `[API Transactions] Auth: Merchant ${payload.merchantId} not active (status: ${merchantDetails[0].status}).`
          );
        ...
        console.warn(
          `[API Transactions] Auth: Merchant ${payload.merchantId} not found or no internalAccountId.`
        );
        throw new UnauthorizedError(
    ...
    console.error(
      "[API Transactions] Auth: DETAILED JWT Verif. Error:",
      "Name:",
      error.name,
      "Msg:",
      error.message,
...
const createTransactionPostSchema = z.object({
  amount: z
    ...
    .regex(
      /^\d+(\.\d{1,2})?$/,
      "Amount must be valid monetary string (e.g., '100.00' or '50')."
    )
    .refine(
      (valStr) => {
        try {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validationResult =
      createTransactionPostSchema.safeParse(requestBodyJson);
    ...
    console.log(
      // Using your original log format
    ...
    const commonTxData = {
      // Data for failed transaction logging
      paymentId: paymentId!,
      amount: amount.toFixed(2),
      type: "Debit" as "Debit" | "Credit", // Ensure type matches enum
    ...
    if (clientReportedPinFailureType === PIN_FAILURE_TYPE_MAX_ATTEMPTS) {
      ...
      console.log(
        `[TX PaymentID: ${paymentId}] Verifying PIN for beneficiary '${beneficiaryDisplayId}'.`
      );
    ...
    if (beneficiaryAccount.status !== "Active") {
      ...
      console.warn(
        `[TX PaymentID: ${paymentId}] ${reason}. Logging failed transaction.`
      );
      await db
        .insert(transactions)
        .values({
    ...
    const result = await db.transaction(async (tx) => {
      ...
      console.log(
        `[TX PaymentID: ${paymentId}] Merchant internal account (${merchantLedgerAccount.id}) validated. Status: ${merchantLedgerAccount.status}, Balance: ${merchantLedgerAccount.balance}`
      );
      ...
      const [updatedCustomerAccountResArray, updatedMerchantAccountResArray] =
        await Promise.all([
          tx
      ...
      if (
        !updatedCustomerAccountRes ||
        !updatedMerchantAccountRes ||
        !debitLeg ||
        !creditLeg
      ) {
        console.error(
          `[TX PaymentID: ${paymentId}] CRITICAL ERROR: Post-update data fetching or leg insertion failed within DB transaction.`
        );
        throw new TransactionProcessingError(
          "Failed to confirm all transaction operations post-update."
        );
      }
    ...
    console.log(
      `[TX PaymentID: ${paymentId}] DB transaction completed successfully. Preparing successful response.`
    );
  ...
  try {
    ...
    if (pageParam) {
      ...
      if (!isNaN(parsedPage) && parsedPage > 0) {
        ...
        console.warn(
          `[API GET Transactions] Invalid page: ${pageParam}. Defaulting to 1.`
        );
      }
    }
    ...
    if (limitParam) {
      ...
      if (!isNaN(parsedLimit) && parsedLimit > 0 && parsedLimit <= 100) {
        ...
        console.warn(
          `[API GET Transactions] Invalid limit: ${limitParam}. Defaulting to 20.`
        );
      }
    }
    ...
    const effectiveStatusFilter =
      statusQueryParam && statusQueryParam.toLowerCase() !== "all"
        ? statusQueryParam
        : null;
    ...
    console.log(
      `[API GET Transactions] Merchant: ${merchantBusinessName} (ID: ${merchantId}). Page: ${page}, Limit: ${limit}, StatusQP: '${statusQueryParam}', EffectiveDBStatus: '${
        effectiveStatusFilter === null ? "All" : effectiveStatusFilter
      }', LegView: CustomerDebit`
    );
    ...
    if (effectiveStatusFilter) {
      whereClauses.push(
        eq(
          transactions.status,
          effectiveStatusFilter as (typeof transactions.status.enumValues)[number]
        )
      );
    }
    ...
    return NextResponse.json({
      data: transactionRecords.map((tx) => ({
        ...
        displayDescription: `Payment from ${
          tx.relatedAccountChildName || tx.relatedAccountDisplayId || "customer"
      ...
      pagination: {
        ...
        statusFilter: statusQueryParam
          ? statusQueryParam.toLowerCase() === "all"
            ? "All"
            : statusQueryParam
          : "All",
      },
    ...
    if (error instanceof ApiError) {
      console.error(
        `[API GET Transactions] ApiError: ${error.message}`,
    ...
    console.error(
      `[API GET Transactions] Unexpected error: ${error.message || "Unknown"}`,
