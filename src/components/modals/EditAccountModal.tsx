...
import { Account, Merchant } from "@/lib/mockData";
...
const updateAccountDetailsAPI = async (
  accountId: string,
  payload: { balance?: number; newPin?: string }
): Promise<Account> => {
  // Create a new payload that matches the balance-and-pin API format
  const apiPayload: { balance?: number; pin?: string } = {};
  ...
  if (payload.newPin) {
    apiPayload.pin = payload.newPin;
  }

  const response = await fetch(`/api/accounts/${accountId}/balance-and-pin`, {
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
...
const EditAccountModal: React.FC<EditAccountModalProps> = ({
  ...
  const handleSaveChanges = async () => {
    ...
    if (balance !== currentBalance) {
      ...
      detailsLog += `Balance changed from ${formatCurrency(
        currentBalance
      )} to ${formatCurrency(balance)}. `;
    }
    if (newPin) {
    ...
    try {
      // The updatedAccountFromServer will now come from the balance-and-pin API
      const updatedAccountFromServer = await updateAccountDetailsAPI(
        account.id,
        payload
      );
      ...
      // Make sure we're preserving any fields that may not be returned by the API
      const mergedAccount = { ...account, ...updatedAccountFromServer };
      onSave(mergedAccount);
      onClose();
    } catch (error) {
      toast.error(
      ...
      case "reactivate":
        return {
          ...
          message: (
            <>
              <p>
                Reactivate account {accountToUpdate.displayId} (
                {tuncateUUID(accountToUpdate.id)}) ({guardianName} /{" "}
                {confirmBeneficiaryName})?
              </p>
              {errorMessage && (
      ...
      <div className="relative mx-auto p-6 border w-full max-w-6xl shadow-lg rounded-md bg-white my-auto">
        <div className="flex justify-between items-center mb-4 pb-3 border-b">
          <h3 className="text-lg font-semibold text-gray-900">
            Account Details - {displayAccountId}
          </h3>
          <button
            onClick={onClose}
              ...
              <dl className="divide-y divide-gray-100 border border-gray-200 rounded-md p-4 text-sm">
                ...
                <div className="py-2 sm:grid sm:grid-cols-3 sm:gap-4 items-center">
                  ...
                  <dd className="mt-1 sm:mt-0 sm:col-span-2">
                    {renderStatusBadge(
                      account.status as DrizzleTransaction["status"],
                      "account"
                    )}
                  </dd>
                </div>
                      ...
                      return (
                        <tr key={tx.id}>
                          ...
                          <td
                            ...
                            title={displayTxnId || ""}
                          >
                            {displayTxnId ? tuncateUUID(displayTxnId) : "N/A"}
                          </td>
                          ...
                          <td
                            className="px-3 py-2 whitespace-nowrap truncate max-w-[200px]"
