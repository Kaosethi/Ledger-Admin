...
export const formatCurrency = (
  ...
  if (isNaN(numAmount)) { // Catches original NaN, undefined, null, or unparseable strings
    return "฿ -.--"; 
  }
  ...
  try {
    ...
    // Fallback for safety, though Intl.NumberFormat is quite robust
...
export const formatDate = (
  ...
  try {
    ...
    return "Error"; // Or a more specific error string
  }
};

export const formatDdMmYyyy = ( // This specifically expects a string or can be adapted
  dateInput: string | Date | null | undefined
): string => {
  if (!dateInput) return "N/A";
  try {
...
export const formatTime = ( // This specifically expects a string or can be adapted
  dateInput: string | Date | null | undefined
): string => {
  if (!dateInput) return "N/A";
  try {
    const date = typeof dateInput === "string" ? new Date(dateInput) : dateInput;
    if (isNaN(date.getTime())) return "Invalid Time"; // Changed from "Invalid Date"
...
export const formatDdMmYyyyHhMmSs = (
  ...
  try {
    ...
    return "Invalid Date/Time"; // More specific error
  }
};
...
export const renderStatusBadge = (
  ...
  let textToDisplay = status || "Unknown"; // Use a different variable name to avoid conflict
  ...
  switch (type) {
    case "account":
      switch (lowerCaseStatus) {
        ...
        case "inactive": bgColor = "bg-gray-100"; textColor = "text-gray-600"; break; // Explicitly handle 'rejected' if needed
    ...
    case "merchant":
      switch (lowerCaseStatus) {
        case "active": bgColor = "bg-green-100"; textColor = "text-green-800"; break;
        case "pending_approval": // Match your enum
        case "pending": bgColor = "bg-blue-100"; textColor = "text-blue-800"; break;
        case "suspended": bgColor = "bg-yellow-100"; textColor = "text-yellow-800"; break;
        case "rejected": bgColor = "bg-red-100"; textColor = "text-red-800"; break;
        default: break;
    ...
    case "transaction":
      switch (lowerCaseStatus) {
        ...
        case "declined": bgColor = "bg-red-100"; textColor = "text-red-800"; break; // Often same as failed
        default: break;
      }
      break;
    // Removed specific 'pending' type case as it's covered by others or default
    default: 
      // Generic status handling if type is not specified or doesn't match
      switch (lowerCaseStatus) {
        ...
        default: break; // Default keeps gray if no match
      }
      break;
  }
...
// Renamed back to tuncateUUID to match existing imports if that's the consensus
export const tuncateUUID = (uuid: string | null | undefined, first = 8, last = 4): string => {
  if (!uuid) return "N/A";
  if (uuid.length <= first + last + 3) { // +3 for "..."
    return uuid; 
  }
