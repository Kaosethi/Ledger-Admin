...
const AccountsTab: React.FC<AccountsTabProps> = ({
  ...
  const [triggerPrint, setTriggerPrint] = useState(false); // Kept triggerPrint state

  // --- Refs ---
  const printComponentRef = useRef<HTMLDivElement>(null); // Original ref setup
  ...
  // --- Print Handler (Original version using useEffect trigger) ---
  const handleActualPrint = useReactToPrint({
    contentRef: printComponentRef, // Using contentRef
    documentTitle: "Selected-Account-QR-Codes",
    onAfterPrint: () => { setTriggerPrint(false) }, // Reset trigger after print
  });
  // Original useEffect to trigger print
  useEffect(() => {
      if (triggerPrint && printComponentRef.current) {
          handleActualPrint();
      } else if (triggerPrint) {
          console.warn("Print triggered but printComponentRef.current is not attached yet.");
          // Optionally reset trigger if ref fails?
          // setTriggerPrint(false);
      }
  }, [triggerPrint, handleActualPrint]); // Depend on triggerPrint
  ...
  // Original print click handler using trigger state
  const handleBulkPrintClick = () => {
    ...
    // Use setTimeout to ensure state update happens before triggering effect
    setTimeout(() => {
        setTriggerPrint(true); // Set state to trigger useEffect
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200"><thead className="bg-gray-50">
            ...
            {filteredAccounts.length === 0 ? ( <tr><td colSpan={8} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">{searchTerm ? 'No accounts match your search.' : 'No accounts found.'}</td></tr> ) : ( filteredAccounts.map((account) => ( <tr key={account.id} className={selectedAccountIds.has(account.id) ? 'bg-blue-50 hover:bg-blue-100' : 'hover:bg-gray-50'}> <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700"><label htmlFor={`select-row-${account.id}`} className="sr-only">Select account {account.id}</label><input id={`select-row-${account.id}`} type="checkbox" className="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded" checked={selectedAccountIds.has(account.id)} onChange={(e) => handleRowSelectChange(account.id, e.target.checked)} /></td> <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{account.id}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{account.name}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{account.guardianName || "N/A"}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{formatCurrency(account.balance)}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{renderStatusBadge(account.status, "account")}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatDate(account.lastTransactionAt)}</td> <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center actions"><button className="text-primary hover:text-secondary focus:outline-none focus:underline" onClick={() => handleViewEditClick(account)}>View/Edit</button></td> </tr> )) )}
          </tbody></table>
      </div>
