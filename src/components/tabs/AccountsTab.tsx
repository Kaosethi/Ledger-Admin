...
const AccountsTab: React.FC<AccountsTabProps> = ({
  ...
  const handleActualPrint = useReactToPrint({
    ...
    // You might need additional polyfills for older browsers if issues persist
  });
  ...
  const handleApplyBulkUpdates = (payload: BulkUpdatePayload) => {
    ...
    setSelectedAccountIds(new Set()); // ADDED: Clear selection
  };
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="overflow-x-auto">
        {/* MODIFIED: Removed potential whitespace between table and thead/tbody */}
              ...
              filteredAccounts.map((account) => (
                <tr key={account.id} className={selectedAccountIds.has(account.id) ? 'bg-blue-50 hover:bg-blue-100' : 'hover:bg-gray-50'}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center actions">
                    <button className="text-primary hover:text-secondary focus:outline-none focus:underline" onClick={() => handleViewEditClick(account)}>
                      View/Edit
                    </button>
                  </td>
                </tr>
