...
const AccountsTab: React.FC<AccountsTabProps> = ({
  ...
  const handleApplyBulkEdit = (
    ...
    if (idsToUpdate.size === 0)
      return alert(
        "No accounts were selected in the modal to apply changes to."
      );
    const updatedAccounts = accounts.map((acc) =>
      idsToUpdate.has(acc.id)
        ? { ...acc, balance: action === "add" ? acc.balance + amount : amount }
        : acc
    );
    ...
    if (logAdminActivity)
      logAdminActivity(
        `Bulk ${action} balance`,
        "Account",
        `${idsToUpdate.size} accounts`,
        `Amount: ${formatCurrency(amount)}. Applied to IDs: ${Array.from(
          idsToUpdate
        ).join(", ")}`
      );
  ...
  const handleBulkPrintClick = () => {
    ...
    if (logAdminActivity)
      logAdminActivity(
        ...
        `Printing QR for all ${accounts.length} visible accounts.`
      );
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
              ...
              accounts.map((account) => (
                <tr key={account.id}>
                  ...
                  <td className="table-cell actions">
                    <button
                      className="text-primary hover:text-secondary view-account-btn"
      ...
      <div className="flex items-center justify-between mt-4">
        <div className="text-sm text-gray-700">
          {" "}
          Showing <span id="pagination-start">1</span> to{" "}
          <span id="pagination-end">{Math.min(10, accounts.length)}</span> of{" "}
          <span id="pagination-total">{accounts.length}</span> accounts{" "}
        </div>
