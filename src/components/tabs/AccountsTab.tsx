...
import React, { useState, useRef, useMemo, useEffect } from "react"; // Removed unused useCallback
// MODIFIED: Import Transaction and Merchant types
...
interface AccountsTabProps {
  ...
  allTransactions: Transaction[]; // ADDED: Prop for all transactions
  merchants: Merchant[]; // ADDED: Prop for merchants (needed by EditAccountModal)
}
...
const AccountsTab: React.FC<AccountsTabProps> = ({
  ...
  merchants, // Destructure new prop
}) => {
  // console.log("AccountsTab: Received accounts prop:", accounts);
  ...
  const handleActualPrint = useReactToPrint({
    contentRef: printComponentRef, documentTitle: "Selected-Account-QR-Codes", onAfterPrint: () => { setTriggerPrint(false) },
  });
  ...
  // Bulk Update Handler (Refactored)
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="overflow-x-auto"> <table className="min-w-full divide-y divide-gray-200"> <thead className="bg-gray-50"> <tr> <th scope="col" className="px-4 py-3 w-12 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> <label htmlFor="select-all-header" className="sr-only">Select all visible accounts</label> <input id="select-all-header" type="checkbox" className="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded" checked={isAllSelected} ref={selectAllCheckboxRef} onChange={handleSelectAllChange} disabled={filteredAccounts.length === 0} /> </th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Account ID</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Child Name</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Guardian Name</th> <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Balance</th> <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th> <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Last Transaction</th> <th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Actions</th> </tr> </thead> <tbody id="accounts-table-body" className="bg-white divide-y divide-gray-200"> {filteredAccounts.length === 0 ? ( <tr> <td colSpan={8} className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center"> {searchTerm ? 'No accounts match your search.' : 'No accounts found.'} </td> </tr> ) : ( filteredAccounts.map((account) => ( <tr key={account.id} className={selectedAccountIds.has(account.id) ? 'bg-blue-50 hover:bg-blue-100' : 'hover:bg-gray-50'}> <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700"> <label htmlFor={`select-row-${account.id}`} className="sr-only">Select account {account.id}</label> <input id={`select-row-${account.id}`} type="checkbox" className="h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded" checked={selectedAccountIds.has(account.id)} onChange={(e) => handleRowSelectChange(account.id, e.target.checked)} /> </td> <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{account.id}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{account.name}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{account.guardianName || "N/A"}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{formatCurrency(account.balance)}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{renderStatusBadge(account.status, "account")}</td> <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatDate(account.lastTransactionAt)}</td> <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center actions"> <button className="text-primary hover:text-secondary focus:outline-none focus:underline" onClick={() => handleViewEditClick(account)}> View/Edit </button> </td> </tr> )) )} </tbody> </table> </div>
      ...
      {/* MODIFIED: Pass allTransactions and merchants to EditAccountModal */}
      <EditAccountModal
        isOpen={isEditAccountModalOpen}
        onClose={() => { setIsEditAccountModalOpen(false); setEditingAccount(null); }}
        onSave={handleSaveAccountChanges}
        account={editingAccount}
        allTransactions={allTransactions} // ADDED PROP
        merchants={merchants} // ADDED PROP
      />
