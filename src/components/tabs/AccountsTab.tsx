...
const AccountsTab: React.FC<AccountsTabProps> = ({
  ...
  const isIndeterminate = useMemo(() => {
    const visibleSelectedCount = filteredAccounts.filter((acc) =>
      selectedAccountIds.has(acc.id)
    ).length;
    return (
      visibleSelectedCount > 0 && visibleSelectedCount < filteredAccounts.length
    );
  }, [filteredAccounts, selectedAccountIds]);
  ...
  const handleApplyBulkUpdates = (payload: BulkUpdatePayload) => {
    ...
    if (idsToUpdate.size === 0) {
      alert("Internal Error: No accounts selected for update.");
      return;
    }
    ...
    idsToUpdate.forEach((accountId) => {
      const originalAccount = accounts.find((acc) => acc.id === accountId);
      if (!originalAccount) {
        console.warn(
          `[BulkUpdate PreCalc] Could not find original account for ID: ${accountId}`
        );
        return;
      }
      let newBalance = originalAccount.balance;
      let newStatus = originalAccount.status;
      let needsUpdate = false;
      if (balanceAction === "add" && amount !== undefined) {
        const calculatedBalance = originalAccount.balance + amount;
        if (calculatedBalance !== originalAccount.balance) {
          newBalance = calculatedBalance;
          needsUpdate = true;
        }
      } else if (balanceAction === "set" && amount !== undefined) {
        if (amount !== originalAccount.balance) {
          newBalance = amount;
          needsUpdate = true;
        }
      }
      if (statusAction === "activate" && originalAccount.status !== "Active") {
        newStatus = "Active";
        needsUpdate = true;
      } else if (
        statusAction === "suspend" &&
        originalAccount.status !== "Suspended"
      ) {
        newStatus = "Suspended";
        needsUpdate = true;
      }
      if (needsUpdate) {
        console.log(
          `[BulkUpdate PreCalc] Account ${accountId} needs update. New Status: ${newStatus}, New Balance: ${newBalance}`
        );
        updatesToApply.set(accountId, {
          ...originalAccount,
          balance: newBalance,
          status: newStatus,
        ...
        console.log(
          `[BulkUpdate PreCalc] Account ${accountId} does not need update.`
        );
      }
    });
    if (updatesToApply.size === 0) {
      console.log("AccountsTab: No actual balance or status changes required.");
      alert("Selected accounts already reflect the requested state.");
      setIsBulkEditModalOpen(false);
      return;
    }
    const finalUpdatedAccounts = accounts.map((acc) =>
      updatesToApply.has(acc.id) ? updatesToApply.get(acc.id)! : acc
    );
    console.log(
      "AccountsTab: Calling onAccountsUpdate with final list (Refactored Logic):",
      finalUpdatedAccounts
    );
    if (onAccountsUpdate) {
    ...
    if (statusAction !== "nochange") {
      logDetails.push(`Status set to ${statusAction}.`);
    }
    if (logAdminActivity) {
      logAdminActivity(
        "Bulk Update Accounts",
        "Account",
        `${updatesToApply.size} accounts changed`,
  ...
  const handleSaveAccountChanges = (updatedAccount: Account) => {
    console.log(
      "AccountsTab: handleSaveAccountChanges called with:",
      updatedAccount
    );
    ...
    console.log(
      "AccountsTab: Calling onAccountsUpdate from single save:",
      updatedAccountsList
    );
    ...
    if (logAdminActivity)
      logAdminActivity(
        ...
        "Account",
        updatedAccount.id,
        `Updated details for ${updatedAccount.name}`
      );
