...
const ActivityLogTab: React.FC<ActivityLogTabProps> = ({ logs = [] }) => {
  ...
  const groupedSessions = useMemo(() => {
    ...
    const sortedLogs = dateFilteredLogs.slice().sort(
    ...
    for (const log of sortedLogs) {
      ...
      if (log.action === "Login Success" || log.action === "Logged In") { // Handle variations
        currentSession = {
          sessionId: log.id,
      ...
      } else if (currentSession && log.adminUsername === currentSession.adminEmail) {
  ...
  return (
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 space-y-3 md:space-y-0">
        <h2 className="text-xl font-semibold text-gray-800">Admin Activity Log</h2>
        <div className="flex flex-wrap items-end gap-2">
          <div>
            <label htmlFor="log-start-date-filter" className="text-xs font-medium text-gray-500 mr-1 block mb-1"> From: </label>
            <input type="date" id="log-start-date-filter" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="block w-auto px-2 py-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary text-sm text-gray-900"/>
          </div>
          <div>
            <label htmlFor="log-end-date-filter" className="text-xs font-medium text-gray-500 mr-1 block mb-1"> To: </label>
            <input type="date" id="log-end-date-filter" value={endDate} onChange={(e) => setEndDate(e.target.value)} min={startDate} className="block w-auto px-2 py-1 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary text-sm text-gray-900"/>
          </div>
        </div>
      </div>
          ...
          paginatedSessions.map((session) => {
            ...
            return (
              <div key={session.sessionId} className="border border-gray-200 rounded-lg overflow-hidden">
                {/* Session Header */}
                <button onClick={() => toggleSession(session.sessionId)} className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 focus:outline-none" aria-expanded={isExpanded} aria-controls={`session-content-${session.sessionId}`}>
                  <div>
                  ...
                  <svg className={`w-5 h-5 text-gray-500 transform transition-transform duration-200 ${isExpanded ? "rotate-180" : "rotate-0"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7" /></svg>
                </button>

                {/* Session Content (Collapsible) */}
                <div id={`session-content-${session.sessionId}`} className={`transition-all duration-300 ease-in-out overflow-hidden ${isExpanded ? "max-h-[1000px] opacity-100" : "max-h-0 opacity-0"}`}>
                      ...
                      <ul className="space-y-2">
                        {session.logs.map((log) => (
                          <li key={log.id} className="text-sm text-gray-700 flex items-start space-x-3">
                            <span className="text-gray-500 whitespace-nowrap">{formatDdMmYyyy(log.timestamp)} {formatTime(log.timestamp)}</span>
                            <span className="flex-1">
                              <span className="font-medium">{log.action}</span>
                              {/* MODIFIED: Display targetType along with targetId */}
                              {(log.targetType || log.targetId) && ( // Show if either exists
                                <span className="text-gray-600">
                                  {" "} - Target: {log.targetType ? `${log.targetType}: ` : ''}{log.targetId || '(System/Unknown)'}
                                </span>
                              )}
                              {log.details && (
                                <span className="block text-xs text-gray-500 pl-2 italic"> → {log.details}</span>
                              )}
                            </span>
                          </li>
                        ))}
      ...
      <div className="flex items-center justify-between mt-6 pt-4 border-t border-gray-200">
        <div className="text-sm text-gray-700">
          Showing <span className="font-medium">{paginationStart}</span> to <span className="font-medium">{paginationEnd}</span> of <span className="font-medium">{totalSessions}</span> sessions
        </div>
        <div className="flex space-x-2">
          <button onClick={handlePrevPage} disabled={currentPage === 1} className="py-1 px-3 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50">Previous</button>
          <button onClick={handleNextPage} disabled={currentPage === totalPages || totalSessions === 0} className="py-1 px-3 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary disabled:opacity-50">Next</button>
        </div>
      </div>
    </div>
  );
