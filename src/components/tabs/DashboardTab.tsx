...
const DashboardTab: React.FC<DashboardTabProps> = ({ accounts = [], merchants = [], transactions = [] }) => {
  ...
  const pendingMerchants = merchants.filter(m => m.status === 'pending_approval').length;
  ...
  const totalTxValue = approvedTransactions.reduce((sum, tx) => sum + (tx.amount || 0), 0);
  const avgTxValue = approvedTransactions.length > 0 ? totalTxValue / approvedTransactions.length : 0;
  // --- End Calculations ---
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
        ...
        <div className="mt-4 md:mt-0 flex flex-col sm:flex-row sm:items-end gap-3 shrink-0">
          <div>
            ...
            <input type="date" id="db-start-date-filter" className="px-3 py-1.5 block w-full sm:w-auto text-sm bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
          </div>
          <div>
            <label htmlFor="db-end-date-filter" className="block text-xs font-medium text-gray-600 mb-1">To:</label>
            <input type="date" id="db-end-date-filter" className="px-3 py-1.5 block w-full sm:w-auto text-sm bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary" />
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 transition duration-300 ease-in-out hover:shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Beneficiaries</span>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 transition duration-300 ease-in-out hover:shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Active Merchants</span>
            <span className="p-2 bg-green-100 rounded-full text-green-600">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"> <path fillRule="evenodd" d="M10.496 2.132a1 1 0 00-.992 0l-7 4A1 1 0 003 7v1.132a1 1 0 00.91.996l.884.066A5.002 5.002 0 0110 12a5 5 0 015.207-4.806l.884-.066A1 1 0 0017 8.132V7a1 1 0 00-.496-.868l-7-4z" clipRule="evenodd" /><path d="M4 13a1 1 0 00-1 1v1a1 1 0 001 1h12a1 1 0 001-1v-1a1 1 0 00-1-1H4z" /></svg>
            </span>
          </div>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 transition duration-300 ease-in-out hover:shadow-lg">
           <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Pending Merchants</span>
        ...
        <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 transition duration-300 ease-in-out hover:shadow-lg">
          <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Transactions</span>
        ...
        {/* Card 5: Total Value */}
         <div className="bg-white rounded-xl shadow-md p-6 border border-gray-200 transition duration-300 ease-in-out hover:shadow-lg">
           <div className="flex items-center justify-between mb-4">
            <span className="text-sm font-semibold text-gray-500 uppercase tracking-wider">Total Value</span>
      ...
      <div className="mt-8 p-6 bg-white rounded-xl shadow-md border border-gray-200">
        ...
        <div className="aspect-[16/6] bg-gray-100 rounded-lg flex items-center justify-center border border-dashed border-gray-300">
          <div className="text-center px-4">
            ...
            <p className="text-sm text-gray-500">Chart visualizations will be displayed here.</p>
            <p className="text-xs text-gray-400 mt-1">(Requires charting library)</p>
          </div>
        </div>
      </div>
