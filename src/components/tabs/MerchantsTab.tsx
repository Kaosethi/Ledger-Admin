...
import type {
  Merchant,
  Transaction,
  Account, // <<<< ADDED Account type import
...
// const tableCellActionsClasses = `${tableCellClasses} text-center`; // Not explicitly used below, but kept for reference

const MerchantsTab: React.FC<MerchantsTabProps> = ({
  merchants = [],
  transactions = [],
  accounts = [], // <<<< DESTRUCTURED accounts prop
  onMerchantsUpdate,
  logAdminActivity,
  ...
  const [confirmActionDetails, setConfirmActionDetails] = useState<{
    ...
    reason?: string; // <<<< ADDED reason to state
  ...
  // Updated to accept and store the optional reason
  const handleRequestConfirm = (
    actionType: AllowedMerchantActionForModal,
    merchant: Merchant,
    reason?: string // <<<< ACCEPTS reason
  ) => {
    setConfirmActionDetails({ actionType, merchant, reason }); // Store reason
  ...
  const handleCloseConfirmModal = () => {
    ...
    setTimeout(
      () => setConfirmActionDetails({ actionType: null, merchant: null, reason: undefined }), // Reset reason
      300
    );
  };

  const handleConfirmAction = async () => {
    const { actionType, merchant, reason } = confirmActionDetails; // Retrieve reason
    ...
    let logDetails = `Status changed for merchant ${merchant.businessName} (${tuncateUUID(merchant.id)}) to ${actionType}`;
    ...
    switch (actionType) {
      case "approve":
        ...
        break;
      case "reject":
        if (rejectMerchant) rejectMerchant(merchant.id, reason, handleCloseConfirmModal); // Pass reason
        else { updateMerchantStatus(merchant, "rejected", reason); handleCloseConfirmModal(); } // Pass reason
        ...
        break;
      case "suspend":
        if (suspendMerchant) suspendMerchant(merchant.id, reason, handleCloseConfirmModal); // Pass reason
        else { updateMerchantStatus(merchant, "suspended", reason); handleCloseConfirmModal(); } // Pass reason
        ...
        break;
      case "reactivate":
        const targetStatusAfterReactivation: BackendMerchantStatus = merchant.status === 'rejected' ? 'pending_approval' : 'active';
        if (reactivateMerchant) reactivateMerchant(merchant.id, handleCloseConfirmModal);
        else { updateMerchantStatus(merchant, targetStatusAfterReactivation); handleCloseConfirmModal(); }
        logActionText = "Reactivate Merchant";
        logDetails = `Reactivated merchant: ${merchant.businessName} (${tuncateUUID(merchant.id)}) to ${targetStatusAfterReactivation}`;
        break;
      default:
    ...
    // For mutations, modal closing is typically handled by react-query's onSuccess/onError in the parent,
    // or the explicit closeModalCallback passed to the mutation function.
    // The handleCloseConfirmModal() in the else blocks above handles non-mutation fallbacks.
  };
  ...
  const updateMerchantStatus = (
      ...
      const updatedMerchants = merchants.map((m) =>
        m.id === merchantToUpdate.id
          ? { 
              ...m, 
              status: newStatus, 
              // Assuming Merchant type expects string dates (updatedAt, submittedAt etc.)
              // as per previous discussion if lib/mockData.ts hasn't fully switched Merchant to Date objects.
              updatedAt: new Date().toISOString(), 
              declineReason: newStatus === 'rejected' 
                             ? (reason || m.declineReason || "Rejected by admin") 
                             : (newStatus === 'active' || newStatus === 'pending_approval' ? null : m.declineReason)
            }
          : m
      );
   ...
   const getConfirmModalProps = () => {
    ...
    switch (actionType) {
      case "approve":
        // No quotes used around merchantName, so no " needed.
      ...
      case "reject":
        if (reason) {
            ...
            message = (
                <>
                    {/* Line 227 area based on new error */}
      ...
      case "suspend":
        if (reason) {
             message = (
                <>
                    {/* Line 239 area based on new error */}
                    <p>Suspend merchant <strong>"{merchantName}"</strong> with reason:</p>
                    {/* Line 240 area based on new error */}
                    <p className="mt-1 text-sm italic text-gray-600">"{reason}"</p>
                </>
            );
        } else {
    ...
    <div className="space-y-8">
      ...
      {selectedMerchant && (
        <MerchantDetailModal
          ...
          onRequestConfirm={handleRequestConfirm} // This now passes the reason for reject/suspend
          merchant={selectedMerchant}
          transactions={transactions} // These are "rich" transactions with Date objects for timestamps
          accounts={accounts}         // <<<< PASSING accounts prop
        />
      )}
      {confirmModalProps && (
        <ConfirmActionModal
