...
const MerchantsTab: React.FC<MerchantsTabProps> = ({
  ...
  const getConfirmModalProps = () => { 
    ...
    switch (actionType) {
      ...
      case "suspend":
        if (reason) { message = (<> <p>Suspend merchant <strong>"{merchantName}"</strong> with reason:</p> <p className="mt-1 text-sm italic text-gray-600">"{reason}"</p> </>); } else { message = <>Suspend merchant <strong>{merchantName}</strong>?</>; }
