  ...
  const handleConfirmAction = () => {
    ...
    switch (actionType) {
      case "approve":
        ...
        else { console.warn("Cannot approve non-pending merchant"); }
        break;
      case "reject":
         if (merchant.status === 'Pending') { newStatus = 'Rejected'; logActionText = "Reject Merchant Application"; }
         else { console.warn("Cannot reject non-pending merchant"); }
        break;
      case "suspend":
         if (merchant.status === 'Active') { newStatus = 'Suspended'; logActionText = "Suspend Merchant"; }
    ...
    if (newStatus) {
        ...
        logAdminActivity( logActionText, "Merchant", merchant.id, `Status changed to ${newStatus} for merchant: ${merchant.name}` );
    } else {
    ...
    const getConfirmModalProps = () => {
        ...
        switch (actionType) {
        case "approve":
            return { title: "Confirm Approval", message: `Are you sure you want to approve the application for "${merchant.name}"?`, confirmButtonText: "Approve", confirmButtonVariant: "success" as const };
        case "reject":
            return { title: "Confirm Rejection", message: ( <> <p className="mb-2"> Are you sure you want to reject the application for “{merchant.name}”? </p> <p className="font-semibold text-red-700"> This action cannot be undone. </p> </> ), confirmButtonText: "Reject Application", confirmButtonVariant: "danger" as const };
        case "suspend":
