...
const MerchantsTab: React.FC<MerchantsTabProps> = ({
  ...
  const handleApprove = (merchantId: string) => {
    ...
    alert(
      `Merchant ${merchantId} approved (simulated). Implement actual logic.`
    );
  };
  ...
  const handleReject = (merchantId: string) => {
    ...
    alert(
      `Merchant ${merchantId} rejected (simulated). Implement actual logic.`
    );
  };
  ...
  const pendingMerchants = merchants.filter(
    (m: Merchant) => m.status === "pending_approval"
  );
  const managedMerchants = merchants.filter(
    (m: Merchant) => m.status !== "pending_approval"
  );
          ...
          >
            <thead className="bg-gray-50">
              <tr>
                ...
                <th scope="col" className="table-header actions">
                  Actions
                </th>
              </tr>
            </thead>
            ...
            >
              {pendingMerchants.length === 0 ? (
                ...
                pendingMerchants.map(
                  (
                    merchant // Removed type assertion, TS infers from filter
                  ) => (
                    <tr key={merchant.id}>
                      ...
                      <td className="table-cell actions space-x-2">
                        ...
                        <button
                          ...
                          className="px-2 py-1 text-xs font-medium text-white bg-green-600 rounded hover:bg-green-700 approve-merchant-btn"
                        >
                          Approve
                        </button>
                        <button
                          onClick={() => handleReject(merchant.id)}
                          className="px-2 py-1 text-xs font-medium text-white bg-red-600 rounded hover:bg-red-700 reject-merchant-btn"
                        >
                          Reject
                        </button>
                        <button
                          onClick={() => handleViewDetails(merchant.id)}
                          className="px-2 py-1 text-xs font-medium text-gray-700 bg-gray-200 rounded hover:bg-gray-300 view-merchant-details-btn"
                        >
                          Details
                        </button>
                      </td>
