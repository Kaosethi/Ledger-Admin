...
const TransactionsTab: React.FC<TransactionsTabProps> = ({
  ...
  const filteredTransactions = useMemo(() => {
    ...
    return transactions.filter(tx => {
      ...
      // MODIFIED: Case-insensitive exact match for merchant ID
  ...
  const handleExport = () => {
    ...
    const csvData = filteredTransactions.map(tx => {
      ...
      return {
        ...
        'Merchant ID': tx.merchantId || 'N/A', // Handle potentially missing merchant ID
    ...
    <div className="bg-white p-6 rounded-lg shadow-md">
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50"><tr><th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Date </th><th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Time </th><th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Merchant Name </th><th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Merchant ID </th><th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Account ID </th><th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"> Transaction ID </th><th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider"> Amount </th><th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider"> Status </th><th scope="col" className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider"> Details </th></tr></thead>
          <tbody id="transactions-table-body" className="bg-white divide-y divide-gray-200">
            {filteredTransactions.length === 0 ? (
              // Ensure the empty row is also clean
              ...
              filteredTransactions.map((transaction: Transaction) => {
                ...
                // MODIFIED: Removed potential whitespace within the returned <tr> for each transaction
                return (
                  <tr key={transaction.id}><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatDdMmYyyy(transaction.timestamp)}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{formatTime(transaction.timestamp)}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{merchantName}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 font-mono">{transaction.merchantId || 'N/A'}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{transaction.accountId}</td><td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{transaction.id}</td><td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 text-center">{formatCurrency(transaction.amount)}</td><td className={`px-6 py-4 whitespace-nowrap text-sm font-medium text-center ${statusClass}`}>{transaction.status}</td><td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-center actions"><button className="text-primary hover:text-secondary view-transaction-btn focus:outline-none focus:underline" onClick={() => handleViewDetailsClick(transaction)} > Details </button></td></tr>
                );
              })
            )}
          </tbody>
